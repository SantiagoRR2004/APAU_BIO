...
class NegativeSelectionVectors:
    ...
    def fit(self, X_normal, X_val=None, y_val=None, pca_2d=None):
        ...
        for gen in range(self.n_generations):
            ...
            combined = np.vstack([self.detectors_, valid_new]) if len(valid_new)>0 else self.detectors_
            ...
            # Plot iteration
            self._plot_generation(X_val if X_val is not None else X_normal,
    ...
    def _plot_generation(self, X_data, y_data, gen, loss, pca_2d=None):
        ...
        if pca_2d is not None and len(X_data) > 0:
            ...
            plt.scatter(det2[:,0], det2[:,1],
                        c='green', marker='X', s=60,
                        label='Detectors')
