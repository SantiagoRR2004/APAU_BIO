...
for i, line in enumerate(lines):
    ...
    raw_data[i, :] = values[:] # All columns (including temperature) is the "raw_data" array
...
class GRUModel(torch.nn.Module):
    def __init__(self,):
        ...
        self.gru1 = torch.nn.GRU(14, 16, batch_first=True, dropout=0.5, num_layers=1)
        self.gru2 = torch.nn.GRU(16, 16, batch_first=True, dropout=0.5, num_layers=1)
        self.dropout = torch.nn.Dropout(0.5)
        self.fc = torch.nn.Linear(16, 1)
        
    def forward(self, x):
        x, _ = self.gru1(x)  # First GRU layer with return_sequences=True (PyTorch always returns all sequences)
        x, _ = self.gru2(x)  # Second GRU layer without return_sequences (just last output is needed)
        x = self.dropout(x[:, -1, :])  # Take the last output from the GRU (equivalent to return_sequences=False)
        x = self.fc(x)
        return x
...
for epoch in range(num_epochs):
    ...
    for samples_train, targets_train in train_dataset:
        ...
        train_mae += np.mean(np.abs(targets_train.numpy().flatten() - outputs_train.detach().cpu().numpy().flatten()))
        batches_train += 1
    ...
    with torch.no_grad():
        for samples_val, targets_val in val_dataset: 
            ...
            val_loss += loss.item()
            val_mae += np.mean(np.abs(targets_val.numpy().flatten() - outputs_val.detach().cpu().numpy().flatten()))
            batches_val += 1
    ...
    print("Epoch {:02d}: loss {:.4f} - train mae {:.4f} - val. loss {:.4f} - val. mae {:.4f}".format(epoch+1, train_loss, train_mae, val_loss, val_mae))
...
plt.xlabel('Epochs')
plt.legend()
plt.savefig("08.GRU.stacked.Loss.png")
...
plt.xlabel('Epochs')
plt.legend()
plt.savefig("08.GRU.stacked.MAE.png")