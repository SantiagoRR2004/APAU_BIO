...
class GRUModel(torch.nn.Module):
    ...
    def forward(self, x):
        x, _ = self.gru1(
            x
        )  # First GRU layer with return_sequences=True (PyTorch always returns all sequences)
        x, _ = self.gru2(
            x
        )  # Second GRU layer without return_sequences (just last output is needed)
        x = self.dropout(
            x[:, -1, :]
        )  # Take the last output from the GRU (equivalent to return_sequences=False)
        x = self.fc(x)
        return x
...
plt.xlabel("Epochs")
plt.legend()
plt.savefig(os.path.join(currentDirectory, "08.GRU.stacked.Loss.png"))
...
plt.xlabel("Epochs")
plt.legend()
plt.savefig(os.path.join(currentDirectory, "08.GRU.stacked.MAE.png"))
