...
def reinforce(
    ...
    for iteration in range(num_iterations):
        ...
        normalized_returns_list = [
            [(G - mean_return) / std_return for G in returns] for returns in all_returns
        ]

        # Compute policy gradients
        policy_loss = 0.0
        for trajectory, normalized_returns in zip(
            trajectories, normalized_returns_list
        ):
        ...
        if (iteration + 1) % 100 == 0:
            print(
                f"Iteration {iteration + 1}/{num_iterations} | Avg Return: {avg_return:.2f}"
            )
