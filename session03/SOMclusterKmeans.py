...
class SOMKohonen:
    ...
    def _cluster_bmus(self, data):
        ...
        self.bmu_cluster_map = {
            tuple(bmu): cluster
            for bmu, cluster in zip(bmu_positions, self.cluster_labels)
        }
    ...
    def plot_clusters(self, train_data, test_data=None):
        ...
        if test_data is not None:
            ...
            test_colors = [
                self.bmu_cluster_map.get(
                    bmu, self.bmu_cluster_map[self._assign_closest_existing_bmu(bmu)]
                )
